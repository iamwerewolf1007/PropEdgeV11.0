#!/usr/bin/env python3
"""
PropEdge V11.0 — Generate Season JSONs
========================================
...
def run_model_on_props(props_df, all_logs, h2h_df, model, player_trust, season_label):
    ...
    for _, prop in props_df.iterrows():
        ...
        if model is not None:
            ...
            d_dt = pd.to_datetime(date_str)
            season_phase = min(max(round(((d_dt.dayofyear - 290) % 365) / 180, 3), 0.0), 1.0)
            fd = {'l30':L30,'l10':L10,'l5':L5,'l3':L3,'volume':vol,'trend':trend,
                  'std10':std10,'defP':defP,'pace_rank':op,'h2h_ts_dev':h2h_ts,
                  'h2h_fga_dev':h2h_fga,'h2h_min_dev':h2h_min,'h2h_conf_weighted':h2h_ts * h2h_cf,
                  ...
                  'fg3a_l10': feats.get('fg3a10', 0.0), 'fg3_pct_l10': feats.get('fg3_pct10', 0.0),
...
def main():
    print("="*60)
    print("PropEdge V11.0 — Generate Season JSONs")
    print("="*60)
    t0 = time.time()
