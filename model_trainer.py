"""
PropEdge V11.0 — Projection Model Trainer
==========================================
...
FEATURES = [
    ...
    'h2h_ts_dev','h2h_fga_dev','h2h_min_dev','h2h_conf_weighted',
    'min_cv','pts_per_min','recent_min_trend','fga_per_min',
    'is_b2b','rest_days','consistency','line',
    'fg3a_l10', 'fg3_pct_l10', 'season_phase'
]
...
def build_training_data(file_2425, file_2526, file_h2h):
    ...
    rolled = pd.concat([
        ...
        sroll('FG3_PCT', 10).rename('_fg3_pct10'),
    ...
    # ── 3-Point & Phase Features ──
    base['fg3a_l10'] = base['_fg3a10'].fillna(0.0).round(1)
    base['fg3_pct_l10'] = np.where(base['_fg3_pct10'] < 1.5, base['_fg3_pct10'] * 100, base['_fg3_pct10'])
    base['fg3_pct_l10'] = base['fg3_pct_l10'].fillna(0.0).round(1)
    base['day_of_year'] = base['GAME_DATE'].dt.dayofyear
    base['season_phase'] = ((base['day_of_year'] - 290) % 365 / 180).clip(0, 1).round(3)
    ...
    def _h2h(row):
        ...
        ts_dev = float(hr.get('H2H_TS_VS_OVERALL', 0.0)) if pd.notna(hr.get('H2H_TS_VS_OVERALL')) else 0.0
        fga_dev = float(hr.get('H2H_FGA_VS_OVERALL', 0.0)) if pd.notna(hr.get('H2H_FGA_VS_OVERALL')) else 0.0
        min_dev = float(hr.get('H2H_MIN_VS_OVERALL', 0.0)) if pd.notna(hr.get('H2H_MIN_VS_OVERALL')) else 0.0
        conf = float(hr.get('H2H_CONFIDENCE', 0.0)) if pd.notna(hr.get('H2H_CONFIDENCE')) else 0.0
        h2h_conf_weighted = ts_dev * conf
        return ts_dev, fga_dev, min_dev, h2h_conf_weighted
    h2h_vals = base.apply(_h2h, axis=1, result_type='expand')
    h2h_vals.columns = ['h2h_ts_dev','h2h_fga_dev','h2h_min_dev','h2h_conf_weighted']
...
def train_and_save(file_2425, file_2526, file_h2h, model_file, trust_file):
    ...
    # Find optimal trees via staged_predict (MSE loss)
    from sklearn.metrics import mean_squared_error
    val_errors = [mean_squared_error(y_val, y_pred) for y_pred in temp_model.staged_predict(X_val)]
    best_n_trees = int(np.argmin(val_errors) + 1)
    print(f"    GBR Early Stopping: {best_n_trees} trees optimal (OOS chron validation)")

    # Retrain final model strictly on training split using best_n_trees
    model = GradientBoostingRegressor(
        n_estimators=best_n_trees, max_depth=4, learning_rate=0.05,
        min_samples_leaf=20, subsample=0.8, random_state=42
    )
    ...
    # Calculate trust on OOS data only (last 10%)
    oos_df = train_df.iloc[split_idx:]
    trust = {
        p: round(float(g['correct'].mean()), 3)
        for p, g in oos_df.groupby('PLAYER_NAME')
        if len(g) >= 3
    }
    with open(trust_file, 'w') as f: json.dump(trust, f, indent=2)
    print(f"    ✓ Trust: {len(trust)} players → {trust_file.name}")
    print(f"    Out-of-sample directional accuracy: {float(oos_df['correct'].mean()):.1%}")
    return model
